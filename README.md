...
路径可以是绝对路径，也可以是相对会话 cwd 的路径；不存在、写重复、或本来就落在 cwd 下，
都不会造成重复或报错。通过 `paths` 扫到的指令，标题用**绝对路径**（`..\..` 链说不清位置），
...
**设置 → 插件 → 插件配置** 里找到本插件那张卡片：逐行增删路径、保存或放弃。
...
- 组合配置里的 `config` 是这一层的**基底**：卡片里「放弃」清掉的是用户覆盖，
  清完就回到 `cordis.patch.yml` 里的值。
