...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = `
      .dsh-icf-card{border:.5px solid var(--dsw-alias-border-l4);background:var(--dsw-alias-bg-layer-3);border-radius:16px;list-style:none;transition:border-color .16s,background .16s}
      .dsh-icf-card:hover{border-color:var(--dsw-alias-label-dimmed)}
      .dsh-icf-cardOpen{background:var(--dsw-alias-bg-layer-2);border-color:var(--dsw-alias-label-dimmed)}
      .dsh-icf-header{appearance:none;width:100%;font:inherit;color:inherit;text-align:left;cursor:pointer;background:0 0;border:0;border-radius:12px;align-items:center;gap:12px;padding:14px 16px;display:flex}
      .dsh-icf-header:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:-2px}
      .dsh-icf-headText{flex-direction:column;flex:1;gap:4px;min-width:0;display:flex}
      .dsh-icf-name{color:var(--dsw-alias-label-primary);font-size:15px;font-weight:600;line-height:1.4}
      .dsh-icf-description{color:var(--dsw-alias-label-tertiary);font-size:13px;line-height:1.5}
      .dsh-icf-pending{align-items:center;border-radius:999px;padding:1px 8px;font-size:11px;line-height:17px;font-weight:500;white-space:nowrap;flex:none;background:var(--dsw-alias-bg-module-platform);color:var(--dsw-alias-label-secondary);display:inline-flex}
      .dsh-icf-chevron{color:var(--dsw-alias-label-tertiary);flex:none;transition:transform .16s}
      .dsh-icf-chevronOpen{transform:rotate(180deg)}
      .dsh-icf-body{border-top:.5px solid var(--dsw-alias-border-l2);margin:0 16px;padding-bottom:8px}
      .dsh-icf-readOnly{color:var(--dsw-alias-label-tertiary);margin:12px 0 0;font-size:12px;line-height:1.5}
      .dsh-icf-field{flex-direction:column;gap:6px;padding:12px 0;display:flex}
    ...
    function Card(props) {
      ...
      return React.createElement(
        'li',
        { className: open ? 'dsh-icf-card dsh-icf-cardOpen' : 'dsh-icf-card' },
        React.createElement(
          'button',
          {
            type: 'button',
            className: 'dsh-icf-header',
            'aria-expanded': open,
            'aria-label': t('title'),
            onClick: () => {
              setOpen(!open)
          ...
          React.createElement(
            'span',
            { className: 'dsh-icf-headText' },
            React.createElement('span', { className: 'dsh-icf-name' }, t('title')),
            React.createElement('span', { className: 'dsh-icf-description' }, t('description')),
          ),
          dirty ? React.createElement('span', { className: 'dsh-icf-pending' }, t('unsaved')) : null,
          React.createElement(Chevron, {
            className: open ? 'dsh-icf-chevron dsh-icf-chevronOpen' : 'dsh-icf-chevron',
          }),
        ),
        open
          ? React.createElement(
              'div',
              { className: 'dsh-icf-body' },
              !snapshot.writable
              ...
              React.createElement(
                ...
                phase === 'failed'
                  ? React.createElement(
                      'p',
                      { className: 'dsh-icf-failed', role: 'status' },
                      t('saveFailed'),
                    )
                  : null,
                React.createElement(
