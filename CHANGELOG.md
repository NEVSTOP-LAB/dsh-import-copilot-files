...
- **工作区之外的路径**（[#2](https://github.com/NEVSTOP-LAB/dsh-import-vscode-ai-files/issues/2)）：
  ...
  路径可绝对可相对 cwd；重复、不存在、或本来就在 cwd 下的路径都不会造成重复或报错。
...
- `npm run check`：8 个文件的 `node --check` + 91 项 `node:test` 全绿
  （glob 9 / frontmatter 9 / discover 22 / 插件 34 / 设置 8 / 客户端 bundle 9）。
  插件那 34 项里包含一条**端到端**：设置服务给出的 `paths` 真的进了发现流程（注入与技能目录），
  以及 schema 装载失败时组合配置继续生效。客户端那 9 项跑的是**真实的 `lib/client.js`**：
  按客户端模块系统的方式执行 bundle，再驱动 `apply(ctx)` 与卡片组件，覆盖注册 key、暂存、
  保存（revision + 回读确认）、只读态、样式安装/卸载。
...
- **还没实测**：卡片在真实 GUI 里出现、保存落盘与生效、「放弃」回到组合配置。
